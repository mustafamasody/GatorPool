...
const About = () => {
    ...
    const meta = {
        ...
        // canonical: 'https://gatorpool.com/about',
        meta: {
            charset: 'utf-8',
            name: {
        ...
        <div className="flex flex-col min-h-screen w-full overflow-y-auto bg-white dark:bg-[#0c0c0c]">
            ...
            <div className="flex flex-col items-center h-full w-full mt-32 px-6">
                ...
                <p className="text-lg mt-3 text-center max-w-screen-lg text-black dark:text-white font-RobotoRegular">
                    GatorPool began as a semester project for the Intro to SWE course at the University of Florida (go gators!).
                    We were given the task of creating an application that provides assistance to underlying causes of mental health issues
                    in early college students. One of the big hassles for college students is finding rides to and from home during weekends and breaks.
                    One of our founders, Hector, presented the idea to us and we thought it was a great way to help students and make some money on the side.
                    Currently on the UF Reddit and Snapchat, everyday there are numerous posts about finding rides to various cities, which we believe our
                    platform can centralize and make it easier for students to find rides.
                </p>
            </div>
            ...
            <div className="flex flex-col items-center h-full w-full mt-16 px-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 mt-16 items-center gap-8">
                    {founders.map((founder, index) => (
                        <div key={index} className="flex flex-col items-center gap-4 bg-slate-100 dark:bg-neutral-900 rounded-xl p-4">
                            <img src={founder.image} alt={founder.name} className="w-48 h-48 rounded-full" />
                            <h2 className="text-2xl text-center text-black dark:text-white font-RobotoBold">{founder.name}</h2>
                            <h3 className="text-lg text-center text-gray-700 dark:text-gray-300 font-RobotoRegular">{founder.role}</h3>
                            <p className="text-lg max-w-96 text-center text-black dark:text-white font-RobotoRegular">{founder.description}</p>
                        </div>
                    ))}
                </div>
            </div>
